...
export class Commands implements vscode.Disposable {
  private EXTENSION_NAME = "sukharev.nand2tetris";
  ...
  constructor() {
    ...
    this.extensionPath = vscode.extensions.getExtension(
      this.EXTENSION_NAME
    ).extensionPath;
    ...
    this.hardwareCmd =
      ...
      symbol +
      this.extensionPath +
      '/bin/lib/Compilers.jar" HardwareSimulatorMain ';
    ...
    this.assemblerCmd =
      ...
      symbol +
      this.extensionPath +
      '/bin/lib/TranslatorsGUI.jar" HackAssemblerMain ';
    ...
    this.CPUCmd =
      ...
      symbol +
      this.extensionPath +
      '/bin/lib/Compilers.jar" CPUEmulatorMain ';

    this.zipSource = JSON.parse(
      fs.readFileSync(this.extensionPath + "/assets/zip.json").toString()
    );
  }
  ...
  public async executeCommand(fileUri: vscode.Uri) {
    ...
    const execName = join(filePath.dir, filePath.name)
      .replace(/ /g, '" "')
      .replace(/\\/g, "/");
    let command: string;
    if (filePath.ext === ".hdl") {
  ...
  public async zipCommand() {
    ...
    let filePath = parse(this.document.fileName)
      .dir.replace(/ /g, '" "')
      .replace(/\\/g, "/");
    const dirArr = filePath
      .split("/")
      .filter((_) => _)
      .reverse();
    ...
    if (inputName == null) {
      vscode.window.showInformationMessage(
        "Could not found source to compress!"
      );
      return;
    }
    ...
    if (runInTerminal) {
      ...
      this.zipCommandInOutputChannel(
        command,
        outputName,
        clearPreviousOutput,
        preserveFocus
  ...
  private executeCommandInOutputChannel(
    ...
    this.process.on("close", (code) => {
      ...
      this.outputChannel.appendLine(
        `[Done] Comparison ${
          this.isSuccess ? `Successfully` : `Failure`
  ...
  private zipCommandInOutputChannel(
    command: string,
    outputName: string,
    clearPreviousOutput,
    preserveFocus
    ...
    this.process.on("close", (code) => {
      ...
      const elapsedTime = (endTime.getTime() - startTime.getTime()) / 1000;
      this.outputChannel.appendLine(
        `[Done] End Compress with code=${code} in ${elapsedTime} seconds`
      );
